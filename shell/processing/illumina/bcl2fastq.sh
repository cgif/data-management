...
LANE=#lane
ILANE=#ilane
REMOVE_ADAPTORS=#removeAdaptors
...
then
	$BCL2FASTQ_HOME/bin/configureBclToFastq.pl --use-bases-mask $BASES_MASK --fastq-cluster-count 0 --mismatches 0 --no-eamss --adapter-sequence $PATH_ADAPTER_DIR/$adapter_r1 --adapter-sequence $PATH_ADAPTER_DIR/$adapter_r2 --input-dir $TMPDIR/$RUN_NAME/Data/Intensities/BaseCalls --sample-sheet $TMPDIR/$RUN_NAME/$FLOWCELL_ID.csv
        ...
        # NO REMOVE ADAPTOR     
	$BCL2FASTQ_HOME/bin/configureBclToFastq.pl --use-bases-mask $BASES_MASK --fastq-cluster-count 0 --mismatches 0 --no-eamss --input-dir $TMPDIR/$RUN_NAME/Data/Intensities/BaseCalls --sample-sheet $TMPDIR/$RUN_NAME/$FLOWCELL_ID.csv
fi
#$BCL2FASTQ_HOME/bin/configureBclToFastq.pl --ignore-missing-control --ignore-missing-stats --ignore-missing-bcl --use-bases-mask $BASES_MASK --fastq-cluster-count 0 --mismatches 0 --no-eamss --input-dir $TMPDIR/$RUN_NAME/Data/Intensities/BaseCalls --sample-sheet $TMPDIR/$RUN_NAME/$FLOWCELL_ID.csv 
